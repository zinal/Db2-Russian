...
Informix Primary Storage Manager доступен начиная с Informix 12.10.
Для более ранних версий Informix используется Informix Storage Manager,
порядок настройки которого описан в соответствующих руководствах.

Простейший вариант организации резервного копирования в современных версиях
Informix основан на хранении резервных копий данных в файловой системе
(на отдельных дисках, сервере NFS/CIFS или на подключенной кластерной
файловой системе). Соответствующий сценарий описан в разделе о резервном
копировании
...
### 2.4.1 Настройка резервного копирования в файловую систему с помощью ontape

Для организации резервного копирования в файловую систему с помощью `ontape`
необходимо:
1. Установить параметры `TAPEDEV` и `LTAPEDEV` в полные пути к существующим
   каталогам, владельцем которых является `informix`.
2. Опционально - установить параметры `BACKUP_FILTER` и `RESTORE_FILTER`.
   В этом случае также потребуется установить параметр `LTAPESIZE`
   (предельный размер файла в килобайтах, должен быть кратным `LTAPEBLK`).
3. Для автоматической архивации логического лога с помощью скрипта
   `alarmprogram.sh` скорректировать его, установив опции `BACKUPLOGS=Y`
   и `BACKUP_CMD="ontape -a -d"`.

Затем можно проверить работоспособность архивирования логов, выполнив
вручную команду:
...
Для выполнения полного резервного копирования можно использовать
следующую команду:
