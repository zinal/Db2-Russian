...
## Переключение вторичного сервера RSS в стандартный режим работы

В случае потери основного сервера для продолжения работы со вторичным сервером RSS его необходимо
переключить в стандартный режим работы и перенаправить на него подключения.

Если с момента потери основного сервера Informix вторичный сервер RSS не останавливался,
достаточно выполнить команду `onmode -d standard`. После выполнения этой команды
произойдёт отключение режима RSS, и бывший вторичный сервер RSS будет готов принимать
пользовательские подключения.

Если вторичный сервер RSS был остановлен, то потребуется выполнить две команды:
1. `onmode -d standard`, для отключения RSS - сервер перейдёт в состояние "Quiescent";
2. `onmode -m`, для открытия соединений - сервер перейдёт в состояние "On-Line".
