...
* `HA_ALIAS` - скорректировать для резервного сервера;
...
на с основном сервере, их будет необходимо применить на резервном сервере из резервной копии
с помощью команды `ontape -l` либо `onbar -r -l`.
